...
use png;
use crate::anim::{AnimFile, BonePose};
use crate::anim_csv::{self, AnimRange};
use crate::anim_xml::{self, AnimObjects, EyeMode};
use crate::model::ModelFile;
use crate::modify;
use crate::pvr::PvrFile;
...
pub fn rk_to_gltf<M: AsRef<Path>, A: AsRef<Path>, O: AsRef<Path>>(
    model_path: M,
    anim_path: Option<A>,
    output_path: O,
) -> io::Result<()> {
    ...
    let mut mf = ModelFile::new(File::open(model_path.as_ref())?);
    ...
    let (mut anim, anim_ranges, anim_objs) = if let Some(anim_path) = anim_path {
        if anim_path.as_ref().extension() == Some(OsStr::new("csv")) {
            let ranges = anim_csv::read_anim_csv(anim_path.as_ref())?;
            let mut af = AnimFile::new(File::open(anim_path.as_ref().with_extension("anim"))?);

            let xml_path = anim_path.as_ref().with_extension("xml");
    ...
    for m in &o.models {
        ...
        let texture_path = model_path.as_ref().with_file_name(&m.material).with_extension("png");
...
//         img.to_png_vec();
    ...
    if let Some(name) = model_path.as_ref().file_stem() {
    ...
    if let Some(anim) = anim {
        for ar in &anim_ranges {
            ...
            for (i, &bone_idx) in bone_nodes.iter().enumerate() {
                ...
                for f in cmp::min(ar.start, anim.frames.len()) .. cmp::min(ar.end, anim.frames.len()) {
    ...
    fs::write(&output_path, gltf_bytes)?;
